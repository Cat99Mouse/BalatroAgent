...
- `agent`: Default mode. Uses the agent prompt profile, agent memory, and read-only observation tools such as `observe_remaining_deck` and `score_candidates`.
- `chatbot`: Uses the chatbot prompt profile, disables observation tools, and renders only the last 10 successful actions as memory. When using `--parallel N`, N workers are spawned. In order to fully utilize you have to have more tasks than workers. Check out th number of tasks that will be created using the `--dry-run` option.
