...
Each strategy is a directory under `src/balatrollm/strategies/` containing metadata, shared action tools, and mode-specific prompt profiles:
...
`agent` mode may expose read-only observation tools and uses `MEMORY.md.jinja`. `chatbot` mode disables observation tools and uses a separate built-in memory renderer that only includes the last 10 successful actions.
...
`chatbot` mode does not render `MEMORY.md.jinja`; it uses a separate built-in memory renderer containing only the last 10 successful actions.
...
Observation tools such as `observe_remaining_deck` and `score_candidates` are injected by the runtime in `agent` mode. They are not defined in `TOOLS.json` and are not available in `chatbot` mode.
...
1. **TOOLS.json**: Define available action tools (usually copied from existing strategies)
1. **agent/STRATEGY.md.jinja**: Define agent strategy philosophy and approach
1. **agent/GAMESTATE.md.jinja**: Format agent game state presentation
1. **agent/MEMORY.md.jinja**: Format agent response history
1. **chatbot/STRATEGY.md.jinja**: Define chatbot strategy philosophy and approach
1. **chatbot/GAMESTATE.md.jinja**: Format chatbot game state presentation
